...
def handle_sales(session_id: str, text: str) -> dict:
    session = load_session(session_id)
    config = load_merged_config(session.get("client_id"), session.get("industry"))

    session["sales_query_raw"] = text.strip()
    save_session(session_id, session)

    llm_response = _llm_sales_response(text, session, config)

    return _result(llm_response, session, config)


def _llm_sales_response(text: str, session: dict, config: dict) -> str:
    customer_name = session.get("customer_name") or "there"
    brand = config.get("brand_name", "the dealership")

    system_prompt = (
        "You are assisting in a vehicle sales enquiry. "
        "Your job is to:\n"
        "1) Acknowledge the customer's interest.\n"
        "2) Ask ONE helpful follow-up question.\n"
